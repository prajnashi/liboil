...
int TEST_downsample2_u8(void)
{
	...
	src = sl_malloc_u8(N);
	dest_ref = sl_malloc_u8(N/2);
	dest_test = sl_malloc_u8(N/2);

	sl_profile_init(t);
	srand(20021003);

	printf("I: " sl_stringify(downsample2_u8_FUNC) "\n");
	...
	for(pass=0;pass<N_PASS;pass++){
		...
		downsample2_u8_ref(dest_ref,src,N);
		sl_profile_start(t);
		downsample2_u8_FUNC(dest_test,src,N);	
		sl_profile_stop(t);

		for(i=0;i<N/2;i++){
			if(dest_test[i] != dest_ref[i]){
				printf("%d %d %d %d %d\n",i,src[i*2],src[i*2+1],dest_ref[i],
					dest_test[i]);
				failures++;
			}
		}
